        ...
        match event {
            ...
            InputEvent::PointerMotion { event, .. } => {
                ...
                if let Some(seat) = shell.seats.for_device(&event.device()).cloned() {
                    ...
                    let fs_info: Option<(f64, bool, u32)> = {
                        let shell = self.common.shell.read();
                        shell.active_space(&output).and_then(|ws| {
                    ...
                    };
                    {
                        ...
                        let (pointer_y, has_fs, sid) = fs_info.unwrap_or((0.0, false, 0));
                        // Save surface_id before update() may reset it.
                        let prev_sid = self.common.fullscreen_reveal.surface_id;
                        let action = self.common.fullscreen_reveal.update(pointer_y, has_fs, sid);
                        match action {
                            RevealAction::Reveal => {
                                self.common
                                    .shell_overlay_state
